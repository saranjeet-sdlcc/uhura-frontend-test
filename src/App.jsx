    ...
    fetchFcmToken().then((token) => {
      if (token) {
        ...
        fetch("http://localhost:4004/device-tokens/register", {
          ...
          .then((res) => res.json())
          .then((data) => console.log("📱 Device token registered:", data))
          .catch((err) => console.error("❌ Device token registration error:", err));
      }
    });
  }, [jwt, userId]);
    ...
    try {
      ...
      chatConnection.on("messageEdited", (msg) => {
        if (msg?.isGroup && msg?.groupId) {
          ...
          updateMessageById(msg.messageId, msg);
        }
      });

      chatConnection.on("messageDeleted", ({ messageId, groupId }) => {
        if (groupId) {
          setGroupMessagesByGroup((prev) => {
            const list = prev[groupId] || [];
            return {
      ...
      chatConnection.on("messageReceipt", ({ groupId, messageId, status, from }) => {
        if (groupId) {
          ...
          updateMessageById(messageId, {
            ...
            deliveredTo:
              status === "delivered"
                ? [...(messages.find((m) => m.messageId === messageId)?.deliveredTo || []), from]
                : messages.find((m) => m.messageId === messageId)?.deliveredTo,
            readBy:
              status === "read"
                ? [...(messages.find((m) => m.messageId === messageId)?.readBy || []), from]
                : messages.find((m) => m.messageId === messageId)?.readBy,
          });
        }
      });
                  ...
                  <>
                    ...
                    {activeTab === "notification" && (
                      <NotificationPanel
                        ...
                        setNotifications={setReceivedNotifications}
                        jwt={jwt}
                        userId={userId}
                      />
                    )}
