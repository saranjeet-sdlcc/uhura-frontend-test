...
export default function CallPanel() {
  ...
  /* ===========================
     ACS helpers
  =========================== */
  async function initACS(token) {
    const client = new CallClient();
    const credential = new AzureCommunicationTokenCredential(token);
    const a = await client.createCallAgent(credential);
    setAgent(a);
    return a;
  }

  async function joinACS(agent, groupId) {
    const c = agent.join(
      { groupId },
      { audioOptions: { muted: false } } // 🔒 IMPORTANT
    );
    setCall(c);
  }
  ...
  async function initiateCall() {
    ...
    const res = await axios.post(
      `${BACKEND_URL}/call/initiate`,
      { calleeUserId: targetUserId, callerLanguage: language },
      { headers: { Authorization: `Bearer ${jwt}` } }
    );

    setBridgeId(res.data.bridgeId);
    const a = await initACS(res.data.acsUser.token);
    await joinACS(a, res.data.groupId);
  }

  async function acceptCall() {
    const res = await axios.post(
      `${BACKEND_URL}/call/accept`,
      { bridgeId: incomingCall.bridgeId, calleeLanguage: language },
      { headers: { Authorization: `Bearer ${jwt}` } }
    );
