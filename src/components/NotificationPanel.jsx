...
export default function NotificationPanel({ jwt, userId, receivedNotifications, onClearReceived }) {
  ...
  const clearNotifications = () => {
    ...
    onClearReceived(); // clears received ones from App state
  };

  // 🔀 merge sent + received
  const allNotifications = [
    ...sentNotifications.map((n) => ({ ...n, source: "SENT" })),
    ...receivedNotifications.map((n) => ({ ...n, source: "RECEIVED" })),
  ...
  return (
    <div className="space-y-6">
      {/* Test Notification Form */}
      <div className="bg-white border rounded-lg p-4">
        <h3 className="font-semibold mb-4 flex items-center">🧪 Test Notification</h3>
      ...
      <div className="bg-white border rounded-lg p-4">
        <div className="flex items-center justify-between mb-4">
          ...
          {allNotifications.length > 0 && (
            <button onClick={clearNotifications} className="text-sm text-red-600 hover:text-red-800">
              Clear All
            </button>
          )}
        </div>
        ...
        {allNotifications.length === 0 ? (
          <p className="text-gray-500 text-center py-4">
            No notifications yet. Send or receive to see them here.
          </p>
        ) : (
          <div className="space-y-3 max-h-96 overflow-y-auto">
            {allNotifications.map((n) => (
              <div
                ...
                className={`border rounded-lg p-3 ${
                  n.source === "SENT" ? "bg-gray-50" : "bg-blue-50"
                }`}
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-gray-500">
                      ...
                      {n.recipientId && ` • To: ${n.recipientId}`}
                    </div>
                  </div>
                </div>
              </div>
